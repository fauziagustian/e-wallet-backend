...
// Witdraw provides a mock function with given fields: input
func (_m *TransactionService) Witdraw(input *dto.WithdrawRequestBody) (*model.Transaction, error) {
	ret := _m.Called(input)

	var r0 *model.Transaction
	var r1 error
	if rf, ok := ret.Get(0).(func(*dto.WithdrawRequestBody) (*model.Transaction, error)); ok {
		return rf(input)
	}
	if rf, ok := ret.Get(0).(func(*dto.WithdrawRequestBody) *model.Transaction); ok {
		r0 = rf(input)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*model.Transaction)
	...
	if rf, ok := ret.Get(1).(func(*dto.WithdrawRequestBody) error); ok {
		r1 = rf(input)
	} else {
		r1 = ret.Error(1)
	}
