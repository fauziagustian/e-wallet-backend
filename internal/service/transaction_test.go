	...
	t.Run("test success get all transactions", func(t *testing.T) {
		...
		transactionRepository.Mock.On("FindAll", 1, &dto.TransactionRequestQuery{}).
			Return([]*model.Transaction{{ID: 1, SourceOfFundID: &sourceOfFundID, SourceOfFund: &model.SourceOfFund{ID: 1, Name: "Cash"},
				UserID: 1, User: model.User{ID: 1, Name: "fauzi", Email: "fauzi@user.com"},
				DestinationID: 1, Wallet: model.Wallet{ID: 1, UserID: 1, User: model.User{ID: 1, Name: "fauzi", Email: "fauzi@user.com"}, Number: "100001", Balance: 10000},
...
func Test_transactionService_Transfer(t *testing.T) {
	...
	t.Run("test success count transaction", func(t *testing.T) {
		...
		transactionRepository.Mock.On("Save", &model.Transaction{UserID: 2, DestinationID: 1, Amount: 500000, Description: "test desc", Category: "Receive Money"}).
			Return(&model.Transaction{ID: 1, UserID: 2, DestinationID: 1, Amount: 500000, Description: "test desc", Category: "Receive Money"}, nil).Once()
		transactionRepository.Mock.On("Save", &model.Transaction{UserID: 1, DestinationID: 2, Amount: 500000, Description: "test desc", Category: "Send Money"}).
			Return(&model.Transaction{ID: 2, UserID: 1, DestinationID: 2, Amount: 500000, Description: "test desc", Category: "Send Money"}, nil).Once()
		...
		input.User = &model.User{ID: 1, Name: "fauzi", Email: "fauzi@user.com"}
		...
		assert.Equal(t, "test desc", transactions.Description)
	})
}

func Test_transactionService_Withdraw(t *testing.T) {
	transactionRepository := mocks.NewTransactionRepository(t)
	walletRepository := mocks.NewWalletRepository(t)
	sourceOfFundRepository := mocks.NewSourceOfFundRepository(t)
	transactionService := NewTransactionService(&TSConfig{TransactionRepository: transactionRepository, WalletRepository: walletRepository, SourceOfFundRepository: sourceOfFundRepository})
	...
	t.Run("test success count transaction", func(t *testing.T) {
		myWallet := &model.Wallet{ID: 1, UserID: 1, Number: "1000001", Balance: 99999999}
		walletRepository.Mock.On("FindByUserId", 1).Return(myWallet, nil).Once()
		transactionRepository.Mock.On("Save", &model.Transaction{UserID: 1, DestinationID: 1, Amount: 500000, Description: "test desc", Category: "Withdraw Money"}).
			Return(&model.Transaction{ID: 1, UserID: 1, DestinationID: 1, Amount: 500000, Description: "test desc", Category: "Withdraw Money"}, nil).Once()
		myWallet.Balance = myWallet.Balance - 500000
		walletRepository.Mock.On("Update", myWallet).Return(myWallet, nil).Once()

		input := &dto.WithdrawRequestBody{}
		input.Amount = 500000
		input.Description = "test desc"
		input.User = &model.User{ID: 1, Name: "fauzi", Email: "fauzi@user.com"}
		transactions, err := transactionService.Witdraw(input)

		assert.Nil(t, err)
		assert.Equal(t, uint(1), transactions.ID)
		assert.Equal(t, uint(1), transactions.UserID)
		assert.Equal(t, uint(1), transactions.DestinationID)
		assert.Equal(t, "Withdraw Money", transactions.Category)
	})
}
