...
type TransactionService interface {
	...
	Witdraw(input *dto.WithdrawRequestBody) (*model.Transaction, error)
...
func (s *transactionService) Witdraw(input *dto.WithdrawRequestBody) (*model.Transaction, error) {
	myWallet, err := s.walletRepository.FindByUserId(int(input.User.ID))
	if err != nil {
		return &model.Transaction{}, err
	}
	...
	transaction.Amount = input.Amount
	transaction.Description = input.Description
	transaction.Category = "Withdraw Money"
	...
	if myWallet.Balance-input.Amount < 0 {
		return transaction, errors.New("your balance not enough")
	}
